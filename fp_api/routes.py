...
# 兼容旧版前端：发票上传接口
@api_router.post("/invoices/upload/{user_id}", response_model=ApiResponse)
async def upload_invoices(
    ...
    """
    try:
        result = await workbench_service.upload_and_create_batch(user_id, files, remark=None)
        return ResponseHelper.success(result, "发票上传成功")
    except HTTPException as e:
        return ResponseHelper.error(e.detail, e.status_code)
    except Exception as e:
