...
export default {
  ...
  computed: {
    ...
    currentImage() {
      ...
      const uid = user.id || ''
      const colorPath = (uid && this.invoice.color_filename)
        ? `${config.API_BASE_URL}/files/${uid}/processed/${encodeURIComponent(this.invoice.color_filename)}`
        : null
  ...
  methods: {
    toAbsoluteUrl(value, folder) {
      ...
      if (String(value).startsWith('/files/')) {
        return `${config.API_BASE_URL}${value}`
      }
      if (folder && this.invoice?.id) {
        const user = JSON.parse(localStorage.getItem('user') || '{}')
        if (user.id) {
          return `${config.API_BASE_URL}/files/${user.id}/${folder}/${encodeURIComponent(value)}`
        }
      }
      return ''
    },
...
.detail-layout {
  ...
  gap: 18px;
  min-height: 72vh;
  animation: detail-fade 0.28s ease-out;
}
